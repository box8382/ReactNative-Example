    ...
    render(){
        ...
        return (
            <ScrollView style={style.root}>
                ...
                <BigCatalogList
                    url={bigUrl}
                    onPress={ (id,title)=>{ this.props.navigation.navigate('MovieDetail', {id,title});} }></BigCatalogList>

                {/* 최신등록순, 평점순, 다운로드순 영화목록 보여주는 작은 사이즈 가로 스크롤 */}
                {/* 2. 최신등록순 */}
                <SubCatalogList title="최신등록순" url={recentUrl} onPress={(id,title)=>this.props.navigation.navigate('MovieDetail',{id,title })}></SubCatalogList>

                {/* 3. 평점순 */}
                <SubCatalogList title="평점순" url={ratingtUrl} onPress={(id,title)=>this.props.navigation.navigate('MovieDetail',{id,title })}></SubCatalogList>

                {/* 4. 다운로드순 */}
                <SubCatalogList title="다운로드순" url={downloadUrl} onPress={(id,title)=>this.props.navigation.navigate('MovieDetail',{id,title })}></SubCatalogList>
